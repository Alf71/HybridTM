...
export async function runRestoreCommand(args: string[]): Promise<void> {
    ...
    const target: RestoreTarget = CliUtils.hasFlag(args, '-create')
        ? await createTargetInstance(args, filePath)
        : getExistingTargetInstance(args);
...
function getExistingTargetInstance(args: string[]): RestoreTarget {
...
async function createTargetInstance(args: string[], filePath: string): Promise<RestoreTarget> {
    ...
    if (!name || !modelName) {
        const header: { name: string; model: string } = await peekBackupHeader(filePath);
...
async function peekBackupHeader(filePath: string): Promise<{ name: string; model: string }> {
