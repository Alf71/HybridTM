...
export class RegistryCommands {
    ...
    static listUsage(): void {
        console.log('Usage: hybridtm list');
    }

    static async runList(args: string[]): Promise<void> {
        if (CliUtils.hasFlag(args, '-help')) {
            RegistryCommands.listUsage();
            return;
        }
    ...
    static async runRemove(args: string[]): Promise<void> {
        if (CliUtils.hasFlag(args, '-help')) {
            RegistryCommands.removeUsage();
            return;
        }
        ...
        if (!CliUtils.hasFlag(args, '-force')) {
            ...
            const confirmed: boolean = await RegistryCommands.confirm('Type "yes" to confirm: ');
            if (!confirmed) {
    ...
    private static async confirm(promptText: string): Promise<boolean> {
